...
def Run_Start_Tasks(self):
    ...
    with cf.ThreadPoolExecutor(max_workers=len(self.start_tasks_list)) as self.st_pool:
        ...
        # Even though it's a crutch, it  really works and takes away the ui freezes hahhahahahhahahh A13
...
def CheckUpdate(self):
    ...
    try:
        self.resp = httpx.get("https://api.github.com/", timeout=10)
    except Exception as e:
...
def CheckVulners(self):
    ...
    if self.validate_vulners_status:
        ...
        result.append([TextChangeAnim, self, self.ui.label_vulners_api_2, "Available"])
    else:
...
def CheckVulnersKey(self):
    ...
    if self.vulners_key in ("", None):
        ...
        result.append([ShowErrMessage, self, "Enter your Vulners.com key in setting and validate it. <a href='https://vulners.com/docs/api_reference/apikey/'>Click for help</a>"])
        return result
...
def CheckLoldrivers(self):
    ...
    if self.validate_loldrivers_status:
        ...
        result.append([TextChangeAnim, self, self.ui.label_loldrivers_2, "Available"])
    else:
